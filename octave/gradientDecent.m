...
for i = 1:4
	...
	[forwardLayer1, forwardLayer2, forwardLayer3] = neuralNetwork([x,y], theta1, theta2);
	...
	% delta3 = target - forwardLayer3; % (1,1)
	% delta2 = delta3 * theta2'; % (1,3)
	% delta2 = delta2(2:3)'; % (2,1)
	costValue = costValue + 0.5 * (target - forwardLayer3) ** 2;

	% jLayer2 = delta3 * forwardLayer2' .* sigmoid(forwardLayer2') .* (1-sigmoid(forwardLayer2'));
	% jLayer11 = delta2(1) * forwardLayer1' .* sigmoid(forwardLayer1') .* (1-sigmoid(forwardLayer1'));
	% jLayer12 = delta2(2) * forwardLayer1' .* sigmoid(forwardLayer1') .* (1-sigmoid(forwardLayer1'));
	% jLayer1 = [jLayer11, jLayer12];
